...
interface AppContextType {
  ...
  addTimetableSlot: (slot: Omit<TimetableSlot, 'id'>) => { success: boolean; conflict?: TimetableConflict };
  updateTimetableSlot: (id: string, updates: Partial<TimetableSlot>) => { success: boolean; conflict?: TimetableConflict };
  deleteTimetableSlot: (id: string) => void;
  batchImportSlots: (slots: Omit<TimetableSlot, 'id'>[]) => void;
  clearTimetable: () => void;

  // Teaching Records (What Was Taught)
  recordLesson: (record: Omit<TeachingRecord, 'id' | 'recordedAt'>) => void;
  updateTeachingRecord: (id: string, updates: Partial<TeachingRecord>) => void;
  deleteTeachingRecord: (id: string) => void;
  toggleParentVisibility: (recordId: string, visible: boolean) => void;
  updateMaterialsUrl: (recordId: string, url: string) => void;
...
const STORAGE_KEY = 'ebda_badr_school_system_state_v1';
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Dynamic Timetable Conflict Detection
  const conflicts = useMemo(() => {
    return detectTimetableConflicts(timetableSlots, teachers, classes, labs, workshops, subjects);
  }, [timetableSlots, teachers, classes, labs, workshops, subjects]);
  ...
  const getTeacherWorkload = (teacherId: string): import('../types').TeacherWorkloadSummary => {
    ...
    const target = teacher?.targetWeeklyLessons || 18;
  ...
  // Timetable Operations
  const addTimetableSlot = (data: Omit<TimetableSlot, 'id'>): { success: boolean; conflict?: TimetableConflict } => {
    ...
    const newConflicts = detectTimetableConflicts(nextSlots, teachers, classes, labs, workshops, subjects);
  ...
  const updateTimetableSlot = (id: string, updates: Partial<TimetableSlot>): { success: boolean; conflict?: TimetableConflict } => {
  ...
  // Teaching Records Operations
  const recordLesson = (data: Omit<TeachingRecord, 'id' | 'recordedAt'>) => {
  ...
  const updateTeachingRecord = (id: string, updates: Partial<TeachingRecord>) => {
  ...
  const updateMaterialsUrl = (recordId: string, url: string) => {
    setTeachingRecords((current) =>
      current.map((r) => (r.id === recordId ? { ...r, materialsUrl: url } : r))
    );
    logActivity('UPDATE', 'teaching_record', recordId, `تحديث رابط المواد التعليمية: ${url}`);
  };
  ...
  const addUser = (data: Omit<User, 'id' | 'createdAt'>) => {
    ...
    setAllUsers((prev) => [...prev, newUser]);
    logActivity('CREATE', 'user', newId, `إنشاء حساب مستخدم جديد: ${data.username} (${data.role})`);
  };

  const updateUser = (id: string, updates: Partial<User>) => {
    const prev = allUsers.find((u) => u.id === id);
    setAllUsers((current) => current.map((u) => (u.id === id ? { ...u, ...updates } : u)));
    logActivity('UPDATE', 'user', id, `تعديل بيانات المستخدم: ${prev?.username || id}`, JSON.stringify(prev), JSON.stringify(updates));
  };
